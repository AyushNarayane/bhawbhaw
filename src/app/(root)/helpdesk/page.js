...
const HelpdeskPage = () => {
  ...
  const handleAddQuery = async () => {
    if (!newQuery.title || !newQuery.message) {
      toast.error("Please provide a title and message for your query.");
      return;
    }
    ...
    try {
      ...
      await setDoc(userRef, { queries: updatedQueries });
  ...
  return (
    <div className="p-6 font-montserrat bg-gray-50 min-h-screen">
      ...
      <div className="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
        <h1 className="text-3xl font-bold text-center mb-6 text-red-600">Helpdesk</h1>

        {/* Query Form */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold mb-4 text-gray-700">Submit a Query</h2>
          <input
            type="text"
          ...
          <button
            ...
            className={`w-full text-white px-4 py-3 rounded-lg font-semibold hover:bg-red-600 transition duration-300 ${isLoading ? 'bg-red-400 cursor-not-allowed' : 'bg-red-500'}`}
            disabled={isLoading}
          >
            Submit Query
          </button>
        ...
        {/* Display Queries */}
        <div>
          <h2 className="text-xl font-semibold mb-4 text-gray-700">Your Queries</h2>
          {queries.length === 0 ? (
            ...
            <div className="space-y-4">
              {queries.map((query) => (
                ...
                >
                  ...
                  <p
                    className={`text-sm font-bold mt-2 ${query.status === "unresolved"
                      ? "text-red-500"
                      : "text-green-500"
                      }`}
                  >
